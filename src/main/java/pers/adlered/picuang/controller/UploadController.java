    ...
    public Result upload(@PathVariable MultipartFile file, HttpServletRequest request) {
        synchronized (this) {
            ...
            if (ToolBox.isPic(suffixName)) {
                ...
                File dest = ToolBox.generatePicFile(suffixName, time);
                ...
                try {
                    ...
                    String url = Prop.get("prefix") + "/images/" + time + filename;
                    result.setCode(200);
                    result.setMsg(url);
    ...
    public Result clone(String url, HttpServletRequest request, HttpSession session) {
        synchronized (this) {
            ...
            try {
                ...
                File dest;
                if (ToolBox.isPic(suffixName)) {
                    dest = ToolBox.generatePicFile(suffixName, time);
                } else {
                    dest = ToolBox.generatePicFile(".png", time);
                }
                Logger.log("Saving into " + dest.getAbsolutePath());
                if (!dest.getParentFile().exists()) {
                    dest.getParentFile().mkdirs();
                ...
                Pattern p = Pattern.compile("(?<=http://|\\.)[^.]*?\\.(com|cn|net|org|biz|info|cc|tv|top)", Pattern.CASE_INSENSITIVE);
                Matcher matcher = p.matcher(url);
                matcher.find();
                result.setData("From " + matcher.group());
                result.setCode(200);
                result.setMsg(Prop.get("prefix") + "/images/" + time + dest.getName());
                int count = Integer.parseInt(Prop.get("imageUploadedCount"));
                ++count;
                Prop.set("imageUploadedCount", String.valueOf(count));
                return result;
